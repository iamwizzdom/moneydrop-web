...
class OfferLoan extends Component {
    ...
    componentDidMount() {
        ...
        if (Utility.isEmpty(loanConsts)) {
            ...
            this.setState({loanTenures: loanConsts['tenure'], interestTypes: loanConsts['interest_type'], purposes: loanConsts['purpose']})
        }
    }

    componentDidUpdate(prevProps: Readonly<P>, prevState: Readonly<S>, snapshot: SS) {

        const {loanConst, loanRequest} = this.props;
        ...
        const {status, title, message} = {...{errors: {}, status: false, title: null, message: null}, ...loanRequest.data};
    ...
    submit = () => {
        ...
        if (Utility.isEmpty(this.state.purpose)) return;
        ...
        if (!Utility.isEmpty(this.state.note)) data.note = this.state.note;
        dispatch(LoanAction.requestLoan(data));
    }
    ...
    render() {
        ...
        const {requesting} = loanRequest;
        const {errors} = {...{errors: {}}, ...loanRequest.data};
                    ...
                    <Card border="light" className={`p-2 border-radius-10`}>
                        <Card.Body>
                            <div className={`text-left`}>
                                <small className={`h5 color-accent font-weight-bold`} style={{lineHeight: 2.3}}>
                                    Request amount
                                </small>
                            ...
                            <div className={`pl-4 pr-4`}>
                                ...
                                <Row className={`mt-4`}>
                                    <div className={`w-100 scroll-horizontal scrollbar-invisible`}>
                                        <div className={`h-100`} style={{width: (118.5 * amountList.length)}}>
                                            {amountList.map((v, k) => {
                                ...
                                <Row className={`pl-lg-5 pr-lg-5`}>
                                    <Col className={`mt-4`}>
                                        <select className="custom-select" onChange={this.setPurpose} defaultValue={``}>
                                            <option value="" disabled>Loan Purpose</option>
                                            {Object.entries(this.state.purposes).map((item, key) => {
                                                return <option key={key} value={item[0]}>{item[1]}</option>;
                                            })}
                                        </select>
                                        <small className={`text-danger`}>{this.state.submitted && (errors.purpose || (Utility.isEmpty(this.state.purpose) && "Please select a loan purpose"))}</small>
                                    </Col>
                                </Row>
                                <Row className={`pl-lg-5 pr-lg-5`}>
                                    <Col className={`mt-4`}>
                                        <Form.Group className={`mt-2 text-left`} controlId="note">
                                            <Form.Label>Note (optional)</Form.Label>
                                            <Form.Control as={`textarea`} rows={5} className={`mt-1`} placeholder={`Enter Note`}
                                                          name={`note`} value={this.state.note} onChange={this.setNote}/>
                                        </Form.Group>
                                        <small className={`text-danger`}>{this.state.submitted && errors.note}</small>
                                    </Col>
                                </Row>
                                <Row className={`justify-content-center`}>
                                    <Button variant="primary" type="submit" onClick={this.submit} className={`font-size-16 min-height-48 mt-5 text-capitalize`}>
                                        {requesting ? <Spinner animation="border" variant="light"/> : 'Make request'}
                                    </Button>
                                </Row>
                            </div>
                        </Card.Body>
