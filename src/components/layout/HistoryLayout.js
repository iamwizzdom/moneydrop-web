        ...
        return <Col md={6} className={`pt-3 pb-3`}>
            <Row>
                ...
                <Col lg={4} md={4} sm={4} xl={4} xs={4} className={`text-right`}>
                    <Row>
                        <Col lg={12} md={12} sm={12} xl={12} xs={12}><small style={{color: theme.color}}>{Utility.format(parseFloat(application.getLoan().getAmount()))}</small></Col>
                        <Col lg={12} md={12} sm={12} xl={12} xs={12}><Badge variant={theme.badge}>{application.getStatus() || "Unknown"}</Badge></Col>
                    </Row>
                </Col>
            </Row>
        </Col>;
