...
class LoanRecipientLayout extends Component {
    ...
    render() {
        ...
        return <Col md={size || 6} className={`pt-4 pb-4 bg-white my-rounded`}>
            <Link className={`text-decoration-none text-dark`} to={{pathname: (application.isReviewed() ? '/user/profile' : '/user/review'), state: {[application.isReviewed() ? 'user' : 'application']: application.isReviewed() ? user : application, from: location}}}>
                <Row style={{padding: 3}}>
