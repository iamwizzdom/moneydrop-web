...
const InfoLayout = (props) => {
    const {title, info, type, showCursor, showEditModal, underline} = props;
    return <Col xl={12} lg={12} md={12} sm={12} xs={12} title={`Click to update`} className={`p-3 ${underline && 'underline'} ${showCursor && 'cursor-pointer'}`} onClick={() => {if (showCursor) showEditModal(type)}}>
...
class UserInfo extends Component {
    ...
    render() {
        ...
        return <>
            <Row>
                <InfoLayout title={`Name`} info={`${user.getFirstname()} ${user.getLastname()}`} showEditModal={showEditModal} showCursor={isMe} type={`name`} underline/>
                {isMe && <InfoLayout title={`Phone`} info={user.getPhone()} showEditModal={showEditModal} showCursor={isMe} type={`phone`} underline/>}
                {isMe && <InfoLayout title={`Email`} info={user.getEmail()} showEditModal={showEditModal} showCursor={isMe} type={`email`} underline/>}
                <InfoLayout title={`Gender`} info={Utility.convertGender(user.getGender(), "Unknown")} showEditModal={showEditModal}  showCursor={isMe} type={`gender`} underline/>
